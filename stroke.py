...
# Forecasting Section
   ...
   # Add insights
    ...
    st.write(f"1. The forecast predicts a {abs(forecast_change):.2f}% {trend} in {forecast_measure} for {forecast_country} over the next {forecast_years} years.")
    ...
    if forecast_df_future['Forecast'].is_monotonic_increasing:
        st.write(f"3. The forecast suggests a consistently increasing trend in {forecast_measure} for {forecast_country}.")
    elif forecast_df_future['Forecast'].is_monotonic_decreasing:
        st.write(f"3. The forecast suggests a consistently decreasing trend in {forecast_measure} for {forecast_country}.")
    else:
        st.write(f"3. The forecast suggests a fluctuating trend in {forecast_measure} for {forecast_country}.")
    ...
    ts_data = filtered_df[(filtered_df['location'] == country_for_decomposition) & (filtered_df['measure'] == selected_measure)]
