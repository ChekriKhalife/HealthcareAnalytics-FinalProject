...
selected_section = st.sidebar.radio("Select Section", 
    ["Introduction", "Geographical Analysis", "Temporal Trends", "Demographic Analysis", "Risk Factor Analysis", 
     "Forecasting", "Risk Factor Impact Simulator", "Statistical Tests"])
...
if selected_section == "Introduction":
    ...
    # Filter data
    filtered_df = filter_data(df, selected_countries, year_range, df['measure'].unique(), df['sex'].unique(), df['age'].unique(), df['rei'].unique())
    
    comp_df = filtered_df.groupby(['location', 'measure'])['Rate'].sum().unstack()

    # Fix 1: Check if the required columns 'Deaths' and 'YLDs' are present
    if 'Deaths' in comp_df.columns and 'YLDs' in comp_df.columns:
        ...
        fig.update_layout(height=600, xaxis_title="Death Rate", yaxis_title="YLDs Rate")
        st.plotly_chart(fig, use_container_width=True)
        ...
        st.write("Unable to create scatter plot: 'Deaths' or 'YLDs' data not available.")
    
    # Fix 2: Use a numeric column like 'Rate' for color in the parallel coordinates plot
    fig_parallel = px.parallel_coordinates(filtered_df, 
                                           color="Rate",  # Use a numeric column here
                                           dimensions=['year', 'Rate'],
                                           color_continuous_scale=px.colors.diverging.Tealrose,
                                           title="Multi-dimensional Comparison of Stroke Burden")
    ...
    if 'Deaths' in comp_df.columns and 'YLDs' in comp_df.columns:
        ...
        st.write("Unable to provide insights on Deaths vs YLDs comparison due to missing data.")
...
# Forecasting Section
    ...
    if st.button("Generate Forecast", key="forecast_filter", help="Click to generate the forecast based on selected filters"):
        ...
        if filtered_df is not None:
                ...
                for measure in selected_measures:
                    ...
                    if len(country_measure_data) > 0:
                        ...
                        if forecast is not None:
                            ...
                            st.error(f"Unable to generate forecast for {country} - {measure}. Please try different parameters or check the data.")
                    else:
                        st.warning(f"No data available for {country} - {measure}")
            ...
            st.error("Unable to generate forecast due to lack of data. Please adjust your filters.")

# Function to perform the forecast
def forecast_data(country_measure_data, forecast_years, selected_risk_factors, metric):
    try:
        # Fit ARIMA model
        model = ARIMA(country_measure_data[metric], order=(5, 1, 0))
        model_fit = model.fit()
